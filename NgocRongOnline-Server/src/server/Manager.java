...
//        Service.createCachePart();
    ...
    public static void sendMap(Session session) {
        Message m = null;
        try {
            m = new Message(-28);
    ...
    public static void sendSkill(Session session) {
        Message m = null;
        try {
            m = new Message(-28);
    ...
    public static void sendItem(Session session) {
        Message m = null;
        try {
            m = new Message(-28);
            m.writer().write(Server.cache[3].toByteArray());
            session.doSendMessage(m);
            m.cleanup();
